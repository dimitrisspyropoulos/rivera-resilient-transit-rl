...
Συνολο ενεργειων: δομικες παρεμβασεις βασισμενες σε αναλυση δικτυων (SNA), μεγιστο μηκος γραμμης 18 κομβοι.
...
# ---------------- Action constraints ----------------
DEFAULT_MAX_ROUTE_LEN = 18   # prevents unrealistic long routes while allowing bridge actions to be feasible
...
def bridge_two_routes(sol, TT_dict, TD_dict=None, max_len=DEFAULT_MAX_ROUTE_LEN):
    """Merge two routes when a feasible bridge exists between them.

    This version keeps the same action logic and constraints, but it searches
    across route pairs instead of trying only one random pair. This makes the
    bridge action much less likely to fail just because an unlucky pair was
    sampled.

    The inserted bridge may be either a direct edge or a shortest-path segment.
    """
    new_sol = copy.deepcopy(sol)
    if len(new_sol.Routes) < 2:
        return sol, 0

    routes = [_route_sanitize(list(r)) for r in new_sol.Routes]
    route_indices = list(range(len(routes)))
    random.shuffle(route_indices)

    best_candidate = None  # (merged_length, idx_a, idx_b, combined_route)

    for idx_a in route_indices:
        for idx_b in route_indices:
            if idx_a == idx_b:
                continue
            ...
            for left, right in candidates:
                ...
                if b in TT_dict.get(a, {}):
                    ...
                    cost, sp = shortestPath(TT_dict, a, b)
                    if not sp or cost == float("inf"):
                        continue
                    bridge = sp

                combined = _route_sanitize(left + bridge[1:] + right[1:])
                if len(combined) > max_len:
                    continue
                if not _route_is_edge_feasible(combined, TT_dict):
                    continue

                candidate = (len(combined), idx_a, idx_b, combined)
    ...
    _, idx_a, idx_b, combined = best_candidate
    new_sol.Routes[idx_a] = combined
    new_sol.Routes.pop(idx_b)
    new_sol.Routes = _routeset_sanitize(new_sol.Routes)
    return new_sol, 1


def shorten_route_via_shortest_path(sol, TT_dict, TD_dict=None, max_len=DEFAULT_MAX_ROUTE_LEN):
    """Replace an internal subpath with a shorter directed shortest path."""
    new_sol = copy.deepcopy(sol)
    ...
    idx1 = random.randint(0, len(route) - 3)
    idx2 = random.randint(idx1 + 2, len(route) - 1)

    cost, sp = shortestPath(TT_dict, route[idx1], route[idx2])
    if not sp or cost == float("inf"):
        return sol, 0

    current_segment_len = idx2 - idx1 + 1
    if len(sp) >= current_segment_len:
        return sol, 0

    new_route = route[:idx1] + sp + route[idx2 + 1:]
    new_route = _route_sanitize(new_route)
    if len(new_route) > max_len:
        return sol, 0
    if not _route_is_edge_feasible(new_route, TT_dict):
        return sol, 0
...
def swap_terminal_to_high_demand_neighbor(sol, TT_dict, TD_dict=None, max_len=DEFAULT_MAX_ROUTE_LEN):
    """Move one terminal to a feasible neighboring node with higher aggregate demand."""
    if TD_dict is None:
        return sol, 0
    ...
    term_idx = random.choice([0, -1])
    old_node = route[term_idx]

    if term_idx == 0:
        anchor = route[1]
        candidates = [n for n in TT_dict.keys() if anchor in TT_dict.get(n, {}) and n not in route[1:]]
    else:
        anchor = route[-2]
        candidates = [n for n in TT_dict.get(anchor, {}).keys() if n not in route[:-1]]

    if not candidates:
        return sol, 0

    def node_demand(n):
        return float(sum(TD_dict.get(n, {}).values())) + float(sum(row.get(n, 0.0) for row in TD_dict.values()))

    best_n = max(candidates, key=node_demand)
    if best_n == old_node:
        return sol, 0

    route[term_idx] = best_n
    new_route = _route_sanitize(route)
    if len(new_route) > max_len:
        return sol, 0
    if not _route_is_edge_feasible(new_route, TT_dict):
        return sol, 0
...
def insert_transfer_hub(sol, TT_dict, TD_dict=None, max_len=DEFAULT_MAX_ROUTE_LEN):
    """Reroute a segment through a high-demand / high-connectivity hub candidate."""
    new_sol = copy.deepcopy(sol)
    if not new_sol.Routes:
        return sol, 0
    ri = random.randint(0, len(new_sol.Routes) - 1)
    ...
    if TD_dict:
        hub_pool = sorted(
            TT_dict.keys(),
            key=lambda n: (len(TT_dict.get(n, {})) + len([u for u in TT_dict if n in TT_dict.get(u, {})]), sum(TD_dict.get(n, {}).values())),
            reverse=True,
        )[:10]
    else:
        hub_pool = sorted(TT_dict.keys(), key=lambda n: len(TT_dict.get(n, {})), reverse=True)[:10]

    hub_pool = [h for h in hub_pool if h not in route]
    if not hub_pool:
        return sol, 0

    hub = random.choice(hub_pool)
    pos = random.randint(1, len(route) - 2)
    u, v = route[pos - 1], route[pos]

    c1, path1 = shortestPath(TT_dict, u, hub)
    c2, path2 = shortestPath(TT_dict, hub, v)
    if not path1 or not path2 or c1 == float("inf") or c2 == float("inf"):
        return sol, 0

    new_segment = path1 + path2[1:]
    new_route = route[:pos - 1] + new_segment + route[pos + 1:]
    new_route = _route_sanitize(new_route)
    if len(new_route) > max_len:
        return sol, 0
    if not _route_is_edge_feasible(new_route, TT_dict):
...
ACTION_FUNCS = {
    "bridge_two_routes": bridge_two_routes,
    "shorten_route_via_shortest_path": shorten_route_via_shortest_path,
    "swap_terminal_to_high_demand_neighbor": swap_terminal_to_high_demand_neighbor,
    "insert_transfer_hub": insert_transfer_hub,
}
...
def train_for_budget_scenario(
    ...
    for ep in range(Episodes):
        ...
        new_sol, ok = ACTION_FUNCS[action](sol, TT_dict, TD_dict)
...
def main():
    ...
    actions = [
        "bridge_two_routes",
        "shorten_route_via_shortest_path",
        "swap_terminal_to_high_demand_neighbor",
        "insert_transfer_hub",
    ]
